...
def start_cassandra_container(ssh):
    ...
    result = ssh("docker run --name biketimer-web-db -d cassandra:3.0")
...
def copy_cassandra_scripts_to_remote_host(cassandra_container_id, ssh_con_str):
    ...
    print "pwd: " + pwd
    local_dir = pwd + "/../web-db/db_migrations"
...
def run_cassandra_migration_scripts_in_docker_container(ssh, container_id):
    ...
    print "Running cassandra migration SEED scripts in docker container."
    result = ssh("docker exec " + container_id + " python /root/db_migrations/migrate.py seed docker")
    print "Migration scripts result:"
    print result
